...
const AddTeamModal = ({ isOpen, onClose, onAddTeam }) => {
  ...
  const searchTeams = async () => {
    try {
      setIsLoading(true);
      console.log(`Searching teams with query: ${searchQuery}`);
      console.log(`API URL: ${import.meta.env.VITE_API_URL}/api/teams/search?query=${searchQuery}`);
      
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/teams/search?query=${searchQuery}`,
