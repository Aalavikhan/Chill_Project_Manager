...
const AddMemberModal = ({ isOpen, onClose, onAddMember }) => {
  ...
  const searchUsers = async () => {
    try {
      ...
      console.log(`API URL: ${import.meta.env.VITE_API_URL}/api/auth/users/search?query=${searchQuery}`);
      
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/auth/users/search?query=${searchQuery}`,
